...
function receivedMessage(event, dh) {
  ...
  Promise.all([dh.getQuestionFlow(), dh.getUserProgress(senderID), dh.getUserResponse(senderID), dh.getUserProfile(senderID), dh.getAccessToken()]).then(function (_ref) {
    ...
    var _userProgress$userPro = userProgress.userProgress,
        ...
        confirmationQid = _userProgress$userPro.confirmationQid;

    expectRespType = expectRespType || 'genesis';
    nextQid = nextQid || 0;
    _responseHandlers.responseHandlerMap[expectRespType](message, event, questionFlow, userProgress, userResponse).then(function (_ref3) {
      var nextQid = _ref3.nextQid,
          isGoingBack = _ref3.isGoingBack;
      ...
      return (0, _MessengerHelper.sendQuestion)(userProfile, nextQid, questionFlow, userResponse).then(function (_ref4) {
        var _ref5 = _slicedToArray(_ref4, 2),
            stopAtQid = _ref5[0],
            nextExpectRespType = _ref5[1];
      ...
      _logger2.default.error('Oops, can not handle user response because ' + err + ' ' + JSON.stringify(err));
      _logger2.default.info('fall back to re-send last question.');
      // can not handle this response, repeat last question
      (0, _MessengerHelper.sendQuestion)(userProfile, nextQid, questionFlow, userResponse).then(function (_ref6) {
        var _ref7 = _slicedToArray(_ref6, 2),
            stopAtQid = _ref7[0],
            nextExpectRespType = _ref7[1];
